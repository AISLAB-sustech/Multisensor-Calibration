...
def Jacbian(delta,SRC,theta_M,p_M,theta_L,p_L,x_i,theta_i,x_iplus1,theta_iplus1):
    '''
    Jacobian matrix of all measurements(MIC,LIDAR, and IMU) w.r.t all parameters(MIC, Lidar, and IMU external parameter and bias of IMU)
    Noted that IMU model havenot been used now.
    ...
    :param x_i:     robot(IMU) position in global frame
    :param theta_i: robot(IMU) oritation in global frame
    ...
    # w.r.t src
    T_i[:2, 3:5] = (R_i @ rot_mat(theta_M)).T / norm(delta_M_i) ** 3 @ np.array(
        [[delta_M_i[1, 0] ** 2, -delta_M_i[0, 0] * delta_M_i[1, 0]],
         [-delta_M_i[0, 0] * delta_M_i[1, 0], delta_M_i[0, 0] ** 2]])

    # w.r.t Lidar parameters
    T_i[2:4, 5:7] = (R_i @ rot_mat(theta_L)).T @ (R_iplus1 - R_i)
    T_i[2:4, 7:8] = rot_mat(theta_L, True).T @ R_i.T @ (R_iplus1 @ p_L + x_iplus1 - R_i @ p_L - x_i)
    return T_i
def bspline(control_points,delta):
    ...
    for segment in range(1, len(control_points) - k + 1):
        ...
        for i in np.arange(0, 1, delta):
            ...
            a =  1*np.array([0, 0, 2 , 6 * i]) @ M @ control_point
            path = np.vstack((path, p))
            v_set = np.vstack((v_set, v))
            a_set = np.vstack((a_set,a))
            yaw = np.arctan2(v[1], v[0])
            yaw_angles = np.append(yaw_angles, yaw)
    w_set = (yaw_angles[1:]-yaw_angles[:-1])/delta
    return path,v_set,yaw_angles,a_set,w_set
...
def constraint_pos2(control_points,pre_point,ext_para,curve_seg):
    ...
    return constraints
def max_negative_min_eigenvalue(control_point,pre_point,ext_para):
    ...
    SRC = ext_para[0][3:5].reshape(-1,1)                  
    p_L = ext_para[1][0:2].reshape(-1,1)                               
    theta_L = ext_para[1][2,0]
    opt_jac = np.zeros((0,6)) 
    
    # jacobian matrix(with only Lidar and MIC external parameter)
    opt_jac = np.zeros((0,6))   
    for i in range(len(path) - 1):
        ...
        opt_jac = v_merge([opt_jac,T_i[3:7,3:9]])

    Full_jac = opt_jac.T @ opt_jac
    _, s, _ = np.linalg.svd(Full_jac)
    save_value(s.min())
    # print(s)
    # rank = np.linalg.matrix_rank(Full_jac)
    # print(rank)
    return -s.min()          # 返回方阵最小特征值的相反数
...
def optimize_bspline(delta,curve_seg,pre_point,ext_para):
    global min_eigen
    if curve_seg == 1:
    ...
    result = minimize(max_negative_min_eigenvalue, control_points,constraints=constraint,bounds=bnds, method="COBYLA")
