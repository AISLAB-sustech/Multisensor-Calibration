...
def profile_perf(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        with open(
            "perf_{}_{}.txt".format(strftime(r'%m_%d-%H_%M_%S'),func.__name__), 'w', encoding="utf-8"
        ) as stream:
            stats = pstats.Stats(pr, stream=stream)
            stats.strip_dirs().sort_stats('tottime').print_stats()
            print("function `{}` calls in {} seconds".format(func.__name__,stats.get_stats_profile().total_tt))
        return result
    return wrapper
...
class Duet(object):
    ...
    >>>     write("output{}.wav".format(i), duet.fs, estimates[i, :]+0.05*duet.x1)
    """
